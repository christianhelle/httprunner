...
download_and_install() {
    ...
    if [ ! -w "$INSTALL_DIR" ]; then
        if command -v sudo >/dev/null 2>&1; then
            ...
            log_info "  INSTALL_DIR=\$HOME/.local/bin curl -fsSL https://christianhelle.com/httprunner/install | bash"
            rm -rf "$temp_dir"
            exit 1
        fi
    else
...
show_usage() {
    ...
    echo "  INSTALL_DIR=\$HOME/.local/bin curl -fsSL https://christianhelle.com/httprunner/install | bash"
    echo ""
    echo "  # Install to custom directory using flag"
    echo "  curl -fsSL https://christianhelle.com/httprunner/install | bash -s -- --dir \$HOME/.local/bin"
}
...
main() {
    ...
    log_info "Documentation: https://christianhelle.com/httprunner/"
}
