...
.EXAMPLE
    ...
    irm https://christianhelle.com/httprunner/install.ps1 | iex -AddToPath $false
#>
...
# Configuration
$GitHubRepo = "christianhelle/httprunner"
$BinaryName = "httprunner.exe"
$ArchiveName = "httprunner-windows-x86_64.zip"
...
function Show-Usage {
    Write-Host ""
    Write-Host "HTTP File Runner Installation Script for Windows" -ForegroundColor "Blue"
    Write-Host ""
    Write-Host "Usage:" -ForegroundColor "Yellow"
    Write-Host "  irm https://christianhelle.com/httprunner/install.ps1 | iex" -ForegroundColor "White"
    Write-Host ""
    ...
    Write-Host "  irm https://christianhelle.com/httprunner/install.ps1 | iex -InstallDir 'C:\Tools'" -ForegroundColor "White"
    Write-Host ""
}
...
function Get-DefaultInstallDir {
    ...
    $candidates = @(
        "$env:LOCALAPPDATA\Programs\httprunner",
    ...
    # Default to LOCALAPPDATA\Programs\httprunner
    return "$env:LOCALAPPDATA\Programs\httprunner"
}
...
function Install-HttpRunner {
    ...
    try {
        ...
        Write-Success "httprunner $Version installed successfully!"
...
function Test-Installation {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "You can now run: httprunner --help"
            return $true
        }
    }
    catch {
